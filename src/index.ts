...
bot.start(async (ctx) => {
  ...
  // 4. Обновляем токен — сохраняем user_id Telegram
  ...
  // 5. Отправляем ссылку для серверной верификации (на серверный endpoint)
  const returnUrl = `https://magicpic-seven.vercel.app/api/auth/telegram/verify?token=${payload}`;

  return ctx.reply(
    '✅ Авторизация прошла успешно!\n\n🔁 Нажмите кнопку ниже, чтобы завершить вход и вернуться на сайт.',
    {
      reply_markup: {
        inline_keyboard: [
          [
